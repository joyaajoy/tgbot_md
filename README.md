...
2. Соберите `.exe`: `npm run build:win`
3. Готовый файл появится в `dist/tgbot_md.exe`. Рядом с ним должен лежать ваш `.env` с `TELEGRAM_BOT_TOKEN` и `GITHUB_TOKEN` для корректной работы бота.
